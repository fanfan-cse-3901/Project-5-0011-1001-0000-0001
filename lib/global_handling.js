...
buttonsOperations.forEach(function(button, index){
  if (arrOps.includes(button.name)) {
    button.addEventListener("click", function() {
      ...
      opPartOne[arrOps.indexOf(button.name)]();
...
let trigPartOne = [function() { if (getAngle() == 'DEG') num = num * (Math.PI / 180); val = Math.sin(num); },
  ...
  function() { val = Math.asin(num); if (getAngle()== 'DEG') val = val * (180 / Math.PI); },
  function() { val = Math.acos(num); if (getAngle()== 'DEG') val = val * (180 / Math.PI); },
  function() { val = Math.atan(num); if (getAngle()== 'DEG') val = val * (180 / Math.PI); }];
...
buttonsTrig.forEach(function(button, index){
  if (arrOps.includes(button.name)) {
    button.addEventListener("click", function() {
      ...
      trigPartOne[arrTrig.indexOf(button.name)]();
     if(/\w/.test(val)){
        lockdown();
      }else {
        commonPartTwo();
        // Part 2 for making displayStr.upper
        displayStr.upper = displayStr.upper + trigPartTwo[arrTrig.indexOf(button.name)]();
        commonPartThree();
      }
    }, false);
  };
...
buttonsExtra.forEach(function(button, index) {
  if (arrOps.includes(button.name)) {
    button.addEventListener("click", function () {
      console.log("test");
      displayStr.lower = arrConstFunc[arrConst.indexOf(button.name)];
  ...
  } else if (arrX.includes(button.name)) {
    button.addEventListener("click", function () {
      commonPartOne();
      arrXPartOne[arrX.indexOf(button.name)]();
      if(/\w/.test(val)){
        lockdown();
      }else {
      commonPartTwo();
      displayStr.upper = displayStr.upper + arrXPartTwo[arrX.indexOf(button.name)]();
      commonPartThree();
      }
    }, false);
  }
...
buttonsExp.forEach(function(button, index){
  if (arrOps.includes(button.name)) {
    button.addEventListener("click", function() {
      commonPartOne();
      arrExpPartOne[arrExp.indexOf(button.name)]();
      if(/\w/.test(val)){
        lockdown();
      }else {
        commonPartTwo();
        displayStr.upper = displayStr.upper + arrExpPartTwo[arrExp.indexOf(button.name)]();
        commonPartThree();
      }
    }, false);
  }
